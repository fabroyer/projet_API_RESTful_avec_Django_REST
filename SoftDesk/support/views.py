...
# Un Viewset concerne un modèle en particulier, gère toutes les opérations du CRUD pour un modèle.
# Une API view est utilisée pour une action précise.
# Ici la ligne queryset fait la même chose que la fonction get_queryset mais sans pouvoir utiliser de filtre.
...
class IssueViewset(ModelViewSet):
    ...
    # on prend l'user de chaque contributeur du projet, il faut être dans cette liste d'user pour que la vue dise au modèle que l'auteur est l'user.
    # Une issue peut être créé que par l'auteur pu le contributeur, si on peut donc la créer l'user sera enregistré comme auteur de l'issue.
    def perform_create(self, serializer):
...
class CommentViewset(ModelViewSet):
    ...
    def get_queryset(self):
        return Comment.objects.filter(
            Q(author=self.request.user) | Q(issue__project__contributor__user=self.request.user))
...
class ContributorViewset(ModelViewSet):
    ...
    # Si on veut les contributeurs, on aura la liste des contributeurs où l'utilisateur est auteur ou contributeur.
    def get_queryset(self):
        return Contributor.objects.filter(
            Q(project__author=self.request.user) | Q(project__contributor__user=self.request.user))
